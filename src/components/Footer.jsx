                ...
                {navLinks.map((link) => (
                  <li key={link.label}>
                    <Link to={link.path} className="ft-link">
                      <span style={{ color: '#63c000', marginRight: 7 }}>›</span>{link.label}
                    </Link>
                  </li>
                ))}
              </ul>
        ...
        <div className="ft-bottom" data-reveal>
          <div style={{ maxWidth: 1280, margin: '0 auto', padding: '18px clamp(20px,4vw,48px)' }} className="ft-bottom-inner">
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 20px', justifyContent: 'center' }}>
              <Link to="/privacy" className="ft-link" style={{ fontSize: '0.78rem' }}>Privacy Policy</Link>
              <Link to="/terms" className="ft-link" style={{ fontSize: '0.78rem' }}>Terms & Conditions</Link>
              <a href="/sitemap.xml" className="ft-link" style={{ fontSize: '0.78rem' }} target="_blank" rel="noopener noreferrer">Sitemap</a>
            </div>
