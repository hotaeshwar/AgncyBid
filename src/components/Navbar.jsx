    ...
    <>
      ...
      <nav className={`nb ${scrolled ? 'nb-scrolled' : 'nb-top'}`} ref={menuRef}>
        ...
        <div className="nb-inner">
          ...
          {/* LEFT - Logo */}
          <Link to="/" className="nb-logo-wrap" onClick={(e) => handleNavClick(e, '#home')}>
            <img
              src="/media/bid.png"
      ...
      <div className={`nb-drawer ${menuOpen ? 'open' : ''}`}>
        ...
        {[...leftLinks, ...rightLinks.filter((l) => !l.external)].map(({ hash, label }) => {
          const id = hash.replace('#', '');
          return (
            <a
              key={hash}
